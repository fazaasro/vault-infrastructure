...
# Index README.md (if exists)
if [ -f "README.md" ]; then
  content=$(cat "README.md")
  sqlite3 "$DB_PATH" "INSERT INTO documents (title, content, path) VALUES ('README.md', '$content', 'README.md');"
  ((total++))
...
if [ -d ".docs" ]; then
  find .docs -name "*.md" -type f | while read -r file; do
    ...
    sqlite3 "$DB_PATH" "INSERT INTO documents (title, content, path) VALUES ('$filename', '$content', '$rel_path');"
    ((total++))
  done
...
for file in DEPLOYMENT*.md; do
  if [ -f "$file" ]; then
    content=$(cat "$file")
    sqlite3 "$DB_PATH" "INSERT INTO documents (title, content, path) VALUES ('$file', '$content', '$file');"
    ((total++))
  done
done
